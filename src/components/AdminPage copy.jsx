...
export default function AdminPage() {
  ...
  const examYearsList = Array.from({length: 17}, (_, i) => (2012 + i).toString());
  ...
  // --- States עבור מנגנון ההעלאה ההמונית ---
  ...
  const handleUpdateCourse = async () => {
    ...
    try {
      ...
      setStatus('idle');
    } catch (e) { alert("שגיאה בעדכון הקורס: " + e.message); setStatus('idle'); }
  };

  const handleResolveReport = async (reportId) => {
    try {
      await set(ref(db, `reported_errors/${reportId}`), null);
    } catch(e) { alert("שגיאה במחיקת הדיווח: " + e.message); }
  };
  ...
  const handleUploadExam = async () => {
    ...
    try {
      ...
      if (missingImagesCount > 0) alert(`✅ המבחן עלה!\n⚠️ שים לב: חסרות תמונות ב-${missingImagesCount} שאלות. נא להעלותן ב"ניהול קיימים".`);
      else alert(`✅ המבחן עלה בהצלחה!`);
      
      setFile(null); setAppendicesFile(null);
    } catch (e) { console.error(e); addLog("❌ שגיאה: " + e.message); setStatus('idle'); }
  };

  // --- הפונקציה החדשה להעלאה המונית (Bulk Upload) ---
  const handleBulkUpload = async () => {
    ...
    for (let i = 0; i < bulkFiles.length; i++) {
        ...
        // ניסיון לחלץ שנה (4 ספרות) מתוך השם
        const yearMatch = filename.match(/(20\d{2})/);
        const extractedYear = yearMatch ? yearMatch[1] : "2026"; // ברירת מחדל אם לא נמצא

        // ניסיון לחלץ מועד מתוך השם
        ...
        try {
            ...
            await set(ref(db, 'uploaded_exams/' + examId), {
              id: examId, studentYear: selectedStudentYear, semester: selectedSemester, course: courseName,
              courseId: selectedCourseId, examYear: extractedYear, examMoed: extractedMoed, title: `${extractedYear} - ${extractedMoed}`,
              questions: questions, hasAppendices: false, parsingMode, uploadedAt: new Date().toISOString()
            });
  ...
  const handleUpdateAppendices = async (examId) => {
    ...
    try {
      ...
      setEditingExamId(null); setNewAppendicesFile(null); setStatus('idle');
  ...
  const handleUploadQuestionImage = async (questionIndex, imageFile) => {
    ...
    try {
      ...
      setExamQuestions(prev => { const newQs = [...prev]; newQs[questionIndex].hasImage = true; return newQs; });
      setStatus('idle');
  ...
  const handleToggleAppeal = async (questionIndex, optionIndex) => {
    try {
      ...
      const newAppeals = currentAppeals.includes(optionIndex) ? currentAppeals.filter(i => i !== optionIndex) : [...currentAppeals, optionIndex];
    ...
    <div className="min-h-screen bg-slate-50 p-6" dir="rtl">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
           <div className="bg-white rounded-3xl shadow-2xl p-6 max-w-md w-full">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div>
                  <select value={editCourseYear} onChange={e => setEditCourseYear(e.target.value)} className="w-full p-3 rounded-xl border border-slate-300">{studentYears.map(y => <option key={y} value={y}>{y}</option>)}</select>
                </div>
                <div>
                  <select value={editCourseSemester} onChange={e => setEditCourseSemester(e.target.value)} className="w-full p-3 rounded-xl border border-slate-300">{semesters.map(s => <option key={s} value={s}>{s}</option>)}</select>
                </div>
              </div>
      ...
      <div className="p-8 max-w-2xl mx-auto bg-white rounded-3xl shadow-xl border border-slate-100">
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl mb-8 overflow-x-auto">
          ...
          <button onClick={() => setActiveTab('manage_exams')} className={`flex-1 p-3 rounded-lg font-bold flex items-center justify-center gap-2 whitespace-nowrap transition ${activeTab === 'manage_exams' ? 'bg-white shadow text-purple-600' : 'text-slate-500'}`}><EditIcon /> ניהול</button>
          ...
          <button onClick={() => setActiveTab('reports')} className={`flex-1 p-3 rounded-lg font-bold flex items-center justify-center gap-2 whitespace-nowrap transition ${activeTab === 'reports' ? 'bg-white shadow text-red-600' : 'text-slate-500'}`}>
            <FlagIcon /> {reportsList.length > 0 && <span className="bg-red-500 text-white text-[10px] px-2 py-0.5 rounded-full mr-1">{reportsList.length}</span>}
          </button>
        </div>
        ...
        {activeTab === 'reports' && (
          <div className="space-y-4 animate-fade-in">
            <h3 className="font-bold text-slate-800 text-xl mb-4">דיווחי סטודנטים</h3>
            {reportsList.length === 0 ? (
              <div className="text-center p-8 bg-slate-50 rounded-2xl border border-dashed border-slate-200 text-slate-400">אין דיווחים כרגע. הכל תקין! 🎉</div>
            ) : (
                ...
                return (
                  <div key={report.id} className="bg-red-50 border border-red-100 p-4 rounded-xl shadow-sm">
                    <div className="flex justify-between items-start mb-2">
                      <div className="text-xs font-bold text-red-800 bg-red-100 px-2 py-1 rounded">{examTitle} • שאלה {report.questionIndex + 1}</div>
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => { setActiveTab('manage_exams'); const examToEdit = examsList.find(e => e.id === report.examId); if (examToEdit) { setSelectedStudentYear(examToEdit.studentYear); setSelectedSemester(examToEdit.semester); setSelectedCourseId(examToEdit.courseId); setTimeout(() => openQuestionsEditor(examToEdit), 500); } else alert("לא ניתן למצוא את המבחן."); }} className="bg-white text-blue-600 border border-blue-200 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-blue-50 transition">עבור לשאלה</button>
                      <button onClick={() => handleResolveReport(report.id)} className="bg-green-100 text-green-700 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-green-200 transition">סמן שטופל</button>
                    </div>
                  </div>
                )
              })
                       ...
                       <div key={exam.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col gap-3">
                         <div className="flex justify-between items-center">
                            ...
                            {(() => {
                               ...
                               if (missingCount > 0) return (<span className="bg-red-100 text-red-700 text-[10px] px-2 py-1 rounded-full font-bold animate-pulse border border-red-200">🚨 חסרות {missingCount} תמונות</span>);
                               return null;
                            })()}
